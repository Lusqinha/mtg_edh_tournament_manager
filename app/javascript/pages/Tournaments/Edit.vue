...
const form = useForm({
  ...
  number_of_rounds: props.tournament.number_of_rounds,
  max_players: props.tournament.max_players,
  tournament_scorings_attributes: props.tournament.tournament_scorings.map(s => ({ ...s, _destroy: false })) || [],
  tournament_achievements_attributes: props.tournament.tournament_achievements.map(a => ({ ...a, _destroy: false })) || []
})

const activeScorings = computed(() => {
  return form.tournament_scorings_attributes.filter(s => !s._destroy)
})

const activeAchievements = computed(() => {
  return form.tournament_achievements_attributes.filter(a => !a._destroy)
})

const addScoring = () => {
  form.tournament_scorings_attributes.push({
    position: activeScorings.value.length + 1,
    points: 0
  })
}

const removeScoring = (index) => {
  const realIndex = form.tournament_scorings_attributes.indexOf(activeScorings.value[index])
  if (form.tournament_scorings_attributes[realIndex].id) {
    form.tournament_scorings_attributes[realIndex]._destroy = true
  } else {
    form.tournament_scorings_attributes.splice(realIndex, 1)
  }
  
  // Reorder positions for active scorings
  activeScorings.value.forEach((scoring, i) => {
...
const removeAchievement = (index) => {
  const realIndex = form.tournament_achievements_attributes.indexOf(activeAchievements.value[index])
  if (form.tournament_achievements_attributes[realIndex].id) {
    form.tournament_achievements_attributes[realIndex]._destroy = true
  } else {
    form.tournament_achievements_attributes.splice(realIndex, 1)
  }
}
      ...
      <form @submit.prevent="submit" class="space-y-8">
        ...
        <div class="space-y-5">
          ...
          <div class="space-y-2">
            ...
            <input v-model="form.name" type="text" id="name" class="w-full px-3 py-2 rounded-md bg-theme-base border border-theme-border focus:border-theme-secondary focus:ring-1 focus:ring-theme-secondary text-theme-text placeholder-theme-muted transition-all outline-none text-sm" required placeholder="Ex: Liga Commander 2025" />
        ...
        <div class="space-y-5">
          ...
          <div class="space-y-3">
            <div v-for="(scoring, index) in activeScorings" :key="index" class="flex items-center gap-4">
        ...
        <div class="space-y-5">
          ...
          <div class="space-y-4">
            <div v-for="(achievement, index) in activeAchievements" :key="index" class="p-4 rounded-md bg-theme-base border border-theme-border space-y-3 relative group">
            ...
            <div v-if="activeAchievements.length === 0" class="text-center p-6 border border-dashed border-theme-border rounded-md text-theme-muted text-sm">
